...
def runCorry(config, files, log, additional=None):
    ...
    """
    cmd = f'./corry_docker -c {config} -o EventLoaderEUDAQ2.file_name={files[0]} -o EventLoaderEUDAQ2:TLU_0.file_name={files[1]} -o EventLoaderHDF5.filename={files[2]} -l {log} -o EventLoaderMuPixTelescope.input_file={files[3]} -o EventLoaderMuPixTelescope.input_directory={"data_docker/telepix2"} -l {log} '
    if additional:
        cmd += additional
    print(cmd)  
    os.system(cmd)
...
def main():
    ...
    #os.system('/usr/bin/bash -c "echo hello world"')    
    #os.system('/usr/bin/bash -c "source /src/root_install/bin/thisroot.sh"')
    ...
    telDir = data_path+'/data_docker/telescope'
    tluDir = data_path+'/data_docker/tlu'
    dutDir = data_path+'/data_docker/dut'
    ...
    for runNmb in range(first, last + 1):
        ...
        telepix_file_found = glob(data_path+f'/data_docker/telepix2/single_run_{runNmb:06}.blck')
