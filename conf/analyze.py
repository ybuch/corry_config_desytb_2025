...
def runCorry(config, files, log, additional=None):
    ...
    """
    cmd = f'./corry_tb -c {config} -o EventLoaderEUDAQ2.file_name={files[0]} -o EventLoaderEUDAQ2:TLU_0.file_name={files[1]} -o EventLoaderHDF5.filename={files[2]} -l {log} -o EventLoaderMuPixTelescope.input_file={files[3]} -l {log} '
    if additional:
        cmd += additional
    print(cmd)  
    os.system(cmd)
...
def main():
    ...
    for runNmb in range(first, last + 1):
        ...
        mask_file = f'/data/dut/module_0/chip_0/run{str(runNmb).zfill(6)}_masked_pixels.txt'
